...
每天自动抓取世界主要一手新闻源，按 **财经 / 政治 / 国际** 分类归档，生成 Markdown 摘要并归档，可选推送到企业微信群机器人或钉钉群机器人。
...
- **推送**：通过 webhook 推送到企业微信 / 钉钉（按 URL 域名自动识别）
...
2. 配置 webhook secret（不配也能跑，只是不会推送通知）：
   ...
   - Name 填 `WEBHOOK_URL`，Value 填你的企业微信 / 钉钉机器人 webhook 地址

3. 手动触发一次验证：
   - 仓库 **Actions** 标签页 → 左侧选 `Daily News Digest` → `Run workflow`
   - 检查 `digests/` 下是否生成当日摘要、机器人是否收到推送
